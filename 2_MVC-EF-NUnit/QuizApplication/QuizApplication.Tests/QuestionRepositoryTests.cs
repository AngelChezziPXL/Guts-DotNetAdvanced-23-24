    ...
    public class QuestionRepositoryTests : DataBaseTests
    {
        ...
        public void _03_GetByCategoryId_ShouldReturnQuestionsInCategory()
        {
            using (var context = CreateDbContext(true))
            {
                ...
                Assert.IsTrue(questionsInCategory.Any(q => q.QuestionString == guid), "Category 99 contains a wrong question.");
                Assert.IsTrue(questionsInCategory.Any(q => q.QuestionString == guid2), "Category 99 contains a wrong question.");
            }
        }
